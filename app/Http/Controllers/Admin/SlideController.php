...
class SlideController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $image = $request->file('image'); //Retrieve the Uploaded image file.
        if(!empty($image)){
            $filename = time() . $image->getClientOriginalName();
            $image->move(public_path('assets/images/slides/'), $filename);
            
            $input['image'] = $filename;
        }

        $slide = Slide::create($input); //Create a New Slide Record

        return redirect()->route('admin.slides.index')->with('success', 'Slide created successfully.'); //Redirect with a Success Message
    }
